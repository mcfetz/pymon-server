...
**Conditions:** `gt`, `ge`, `lt`, `le`, `eq`, `ne`, `between`, `outside`, `no_data`
...
- `no_data`: fires when no matching metric has been received for `threshold` seconds;
  the metric must have been received at least once. Uses server receive time and
  exact metric names.
...
| `cert_valid` | `<domain>:remaining_days` (days until certificate expiry) |
